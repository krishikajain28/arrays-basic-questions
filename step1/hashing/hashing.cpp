...
int main() {
    ...
    for(int i = 0; i<13; i++){
        if(hasharray[i] > 0 ){
        if ( hasharray[i] < minFreq ) { minFreq = hasharray[i]; minNum = i; }
        if ( hasharray[i] > maxFreq ) { maxFreq = hasharray[i]; maxNum = i; } 
        }
    }
    
    cout << "The element " << minNum << " is the number with min freq: " << minFreq; 
    cout << "\nThe element " << maxNum << " is the number with max freq: " << maxFreq;
